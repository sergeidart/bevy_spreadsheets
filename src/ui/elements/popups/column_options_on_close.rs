...
pub(super) fn handle_on_close(
    ...
    if needs_save {
        ...
        if popup_sheet_name.starts_with("__virtual__") {
                    ...
                    if let Some(parent_link) = &vmeta.structure_parent {
                        ...
                        let schema_updates: Vec<StructureFieldDefinition> = vmeta
                            ...
                            .map(|vcol| StructureFieldDefinition {
                                ...
                                structure_key_parent_column_index: vcol.structure_key_parent_column_index,
                        ...
                        {
                            if let Some(parent_sheet) = registry.get_sheet_mut(&parent_cat, &parent_sheet_name) {
                                if let Some(parent_meta) = &mut parent_sheet.metadata {
                                    if let Some(parent_col) = parent_meta.columns.get_mut(parent_col_index) {
                                        if parent_col.structure_schema.is_none() {
                                            parent_col.structure_schema = Some(Vec::new());
                                        }
                                        let mut schema = parent_col.structure_schema.take().unwrap_or_default();
                                        ...
                                        for (i, upd) in schema_updates.iter().enumerate() {
                                            if let Some(field) = schema.get_mut(i) {
                                                ...
                                                field.structure_schema = upd.structure_schema.clone();
                                                field.structure_column_order = upd.structure_column_order.clone();
                                                field.structure_key_parent_column_index = upd.structure_key_parent_column_index;
                                                field.structure_ancestor_key_parent_column_indices = upd.structure_ancestor_key_parent_column_indices.clone();
                                            }
                                        }
            ...
            warn!("Edited virtual sheet '{}' but could not propagate changes to parent.", popup_sheet_name);
        } else {
            ...
            if let Some(data_to_save) = registry.get_sheet(&popup_category, &popup_sheet_name) {
                if let Some(meta_to_save) = &data_to_save.metadata {
                    info!("Filter/Context changed for '{:?}/{}', triggering save.", popup_category, popup_sheet_name);
                    save_single_sheet(registry, meta_to_save);
                } else {
