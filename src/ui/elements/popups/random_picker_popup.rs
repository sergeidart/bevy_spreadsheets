...
pub fn show_random_picker_popup(
    ...
    if ui_result.apply_clicked {
        ...
        if !sheet_name.is_empty() {
            ...
            if let Some(sheet) =
                registry.get_sheet_mut(&state.options_column_target_category, &sheet_name)
            {
                ...
                if sheet.metadata.is_none() {
                    ...
                    sheet.metadata = Some(SheetMetadata::create_generic(
                        sheet_name.clone(),
                        default_filename,
                        num_cols,
                ...
                if let Some(meta) = &mut sheet.metadata {
                    ...
                    let weight_cols: Vec<usize> = state
                        .random_picker_weight_columns
                        .iter()
                        .filter_map(|o| *o)
                        .collect();
                    let summ_cols: Vec<usize> = state
                    ...
                    let settings = RandomPickerSettings {
                        ...
                        weight_exponents: state
                            .random_picker_weight_exponents
                            .iter()
                            .cloned()
                            .take(weight_cols.len())
                            .collect(),
                        weight_multipliers: state
    ...
    if ui_result.cancel_clicked || ui_result.close_via_x {
        state.show_random_picker_panel = false;
    }
}
