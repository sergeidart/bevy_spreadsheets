...
use bevy_egui::egui;
use crate::ui::elements::editor::state::{EditorWindowState, AiModeState};
...
pub fn show_ai_prompt_popup(
    ...
    if !state.show_ai_prompt_popup { return; }
    ...
    egui::Window::new("AI Prompt")
        ...
        .show(ctx, |ui| {
            ...
            ui.add_sized([ui.available_width(), 120.0], egui::TextEdit::multiline(&mut state.ai_prompt_input).hint_text("e.g. Give me list of games released this month"));
            ui.horizontal(|ui| {
                if ui.add_enabled(!state.ai_prompt_input.trim().is_empty() && session_api_key.0.is_some(), egui::Button::new("Send"))
                    .on_hover_text(if session_api_key.0.is_none() { "API key missing" } else { "Send prompt to AI" })
                    .clicked() { do_send = true; }
    ...
    if do_send {
        ...
        if let Some(sheet_meta) = registry.get_sheet(&task_category, &sheet_name).and_then(|d| d.metadata.as_ref()) {
        ...
        if let Ok(pretty) = serde_json::to_string_pretty(&payload) {
            state.ai_raw_output_display = format!("--- AI Prompt Payload ---\n{}", pretty);
        }
        ...
        runtime.spawn_background_task(move |mut ctx| async move {
            let api_key_value = match api_key_for_task { Some(k) if !k.is_empty() => k, _ => {
                ...
                ctx.run_on_main_thread(move |world_ctx| { world_ctx.world.commands().entity(commands_entity).insert(SendEvent::<AiBatchTaskResult>{ event: AiBatchTaskResult { original_row_indices: Vec::new(), result: Err(err_msg), raw_response: None, included_non_structure_columns: Vec::new(), key_prefix_count: 0, prompt_only: true } }); }).await; return; } };
