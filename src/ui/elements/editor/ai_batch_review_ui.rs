...
pub(super) fn draw_ai_batch_review_panel(
    ...
    ui.horizontal(|ui| {
        ...
        ui.add_space(12.0);
        let has_actionable = !state.ai_row_reviews.is_empty() || state.ai_new_row_reviews.iter().any(|nr| nr.duplicate_match_row.is_none() || (nr.duplicate_match_row.is_some() && nr.merge_decided));
        let accept_all_enabled = has_actionable && !state.ai_batch_has_undecided_merge;
        let accept_btn = ui.add_enabled(accept_all_enabled, egui::Button::new(RichText::new("Accept All").strong()));
        if accept_btn.clicked() && accept_all_enabled { accept_all_clicked_header = true; }
        if !accept_all_enabled {
            let mut reason = String::new();
            if state.ai_batch_has_undecided_merge { reason.push_str("Pending Merge/Separate decisions (press Decide). "); }
            if !has_actionable { reason.push_str("No changes to accept."); }
            if !reason.is_empty() { accept_btn.on_hover_text(reason); }
        } else { accept_btn.on_hover_text("Apply all AI and merge decisions"); }
        let decline_btn = ui.button(RichText::new("Decline All").color(Color32::LIGHT_RED));
        if decline_btn.clicked() { decline_all_clicked_header = true; }
    ...
    enum RowBlock {
        Separator,
        ExistingOrig(usize), ExistingAi(usize), ExistingChoices(usize),
        NewAi(usize), NewMarker(usize),
        DupOrig(usize), DupAi(usize), DupChoices(usize)
    }
    ...
    for (i, _rr) in state.ai_row_reviews.iter().enumerate() {
        push_group(vec![RowBlock::ExistingOrig(i), RowBlock::ExistingAi(i), RowBlock::ExistingChoices(i)], &mut blocks, &mut group_start_indices);
    }
    for (i, nr) in state.ai_new_row_reviews.iter().enumerate() {
        if nr.duplicate_match_row.is_some() {
            push_group(vec![RowBlock::DupOrig(i), RowBlock::DupAi(i), RowBlock::DupChoices(i)], &mut blocks, &mut group_start_indices);
        } else {
            push_group(vec![RowBlock::NewAi(i), RowBlock::NewMarker(i)], &mut blocks, &mut group_start_indices);
        }
    }
        ...
        builder.header(22.0, |mut header| {
            ...
            for (bi, blk) in blocks.iter().enumerate() {
                ...
                match blk {
                    RowBlock::Separator => {
                        ...
                        body.row(6.0, |mut row| {
                            for _c in 0..total_cols { row.col(|ui| {
                                let rect = ui.max_rect();
                                let y = rect.center().y;
                                ui.painter().hline(rect.left()..=rect.right(), y, egui::Stroke::new(1.0, Color32::from_gray(90))); // subtle gray separator
                            }); }
                        });
                ...
                body.row(row_height, |mut row| {
                    ...
                    let mut finish_cell = |_ui: &mut egui::Ui| { col_i += 1; };
                    match blk {
                        RowBlock::Separator => { /* already handled */ }
                        RowBlock::ExistingOrig(idx) => {
                            if let Some(rr) = state.ai_row_reviews.get(*idx) {
                                // control column
                                row.col(|ui| { if ui.button("Accept").clicked() { existing_accept.push(*idx); } finish_cell(ui); });
                        ...
                        RowBlock::ExistingAi(idx) => {
                            if let Some(rr) = state.ai_row_reviews.get_mut(*idx) {
                                row.col(|ui| { if ui.button("Cancel").clicked() { existing_cancel.push(*idx); } finish_cell(ui); });
                                for (_hdr, value) in &ancestor_key_columns { row.col(|ui| { ui.label(RichText::new(value).color(Color32::LIGHT_GREEN)); finish_cell(ui); }); }
                                for actual_col in &union_cols { row.col(|ui| {
                        ...
                        RowBlock::ExistingChoices(idx) => {
                            if let Some(rr) = state.ai_row_reviews.get_mut(*idx) {
                                row.col(|ui| { ui.add_space(2.0); finish_cell(ui); });
                                for _ in &ancestor_key_columns { row.col(|ui| { ui.add_space(2.0); finish_cell(ui); }); }
                                for actual_col in &union_cols { row.col(|ui| {
                                    if let Some(pos) = rr.non_structure_columns.iter().position(|c| c == actual_col) {
                                        let orig_val = rr.original.get(pos).cloned().unwrap_or_default();
                                        let ai_val = rr.ai.get(pos).cloned().unwrap_or_default();
                                        if orig_val == ai_val { ui.small(RichText::new("Same").color(Color32::GRAY)); }
                                        else {
                        ...
                        RowBlock::NewAi(idx) => {
                            if let Some(_nr) = state.ai_new_row_reviews.get_mut(*idx) {
                                row.col(|ui| { if ui.button("Accept").clicked() { new_accept.push(*idx); } finish_cell(ui); });
                                for _ in &ancestor_key_columns { row.col(|ui| { ui.add_space(0.0); finish_cell(ui); }); }
                                for (i, _actual_col) in union_cols.iter().enumerate() { row.col(|ui| { if i == 0 { ui.colored_label(Color32::LIGHT_BLUE, "AI Added"); } else { ui.label(""); } finish_cell(ui); }); }
                            }
                        }
                        RowBlock::NewMarker(idx) => {
                            if let Some(nr) = state.ai_new_row_reviews.get_mut(*idx) {
                                row.col(|ui| { if ui.button("Cancel").clicked() { new_cancel.push(*idx); } finish_cell(ui); });
                                for (_hdr, value) in &ancestor_key_columns { row.col(|ui| { ui.label(RichText::new(value).color(Color32::LIGHT_GREEN)); finish_cell(ui); }); }
                                for actual_col in &union_cols { row.col(|ui| {
                                    ...
                                    for actual_col in &union_cols { row.col(|ui| {
                                        if let Some(pos) = nr.non_structure_columns.iter().position(|c| c == actual_col) {
                                            ...
                                            if nr.merge_decided && nr.merge_selected {
                                                if let Some(choices) = nr.choices.as_ref() { if let Some(choice) = choices.get(pos) { if matches!(choice, ReviewChoice::AI) { ui.label(RichText::new(orig_val).strikethrough()); finish_cell(ui); return; } } }
                                            }
                                            ui.label(orig_val);
                                        } else { ui.label(""); }
                                        finish_cell(ui);
                                    }); }
                                } else { for _ in &union_cols { row.col(|ui| { ui.label("?"); finish_cell(ui); }); } }
                            }
                        }
                        RowBlock::DupAi(idx) => {
                            if let Some(nr) = state.ai_new_row_reviews.get_mut(*idx) {
                                    ...
                                    if let Some(pos) = nr.non_structure_columns.iter().position(|c| c == actual_col) {
                                        if nr.merge_selected {
                                            if let (Some(orig_vec), Some(choices)) = (nr.original_for_merge.as_ref(), nr.choices.as_mut()) {
                                                let orig_val = orig_vec.get(pos).cloned().unwrap_or_default();
                                                let ai_val = nr.ai.get(pos).cloned().unwrap_or_default();
                                                let norm = |s:&str| s.replace(['\r','\n'], "").to_lowercase();
                                                if norm(&orig_val) == norm(&ai_val) { ui.small(RichText::new("Same").color(Color32::GRAY)); }
                                                else {
                                                    let mut choice = choices.get(pos).cloned().unwrap_or(ReviewChoice::Original);
                                                    if ui.radio_value(&mut choice, ReviewChoice::Original, "Orig").clicked() { if pos < choices.len() { choices[pos] = ReviewChoice::Original; } }
                                                    if ui.radio_value(&mut choice, ReviewChoice::AI, "AI").clicked() { if pos < choices.len() { choices[pos] = ReviewChoice::AI; } }
                                                }
                                            }
                                        } else { ui.small(RichText::new("Separate").color(Color32::DARK_GRAY)); }
                                    } else { ui.label(""); }
                                    finish_cell(ui);
                                }); }
                            }
    ...
    state.ai_batch_has_undecided_merge = state.ai_new_row_reviews.iter().any(|x| x.duplicate_match_row.is_some() && !x.merge_decided);

    if accept_all_clicked_header {
        // Existing rows (always actionable)
        let existing_len = state.ai_row_reviews.len();
        for idx in 0..existing_len { if let Some(rr) = state.ai_row_reviews.get(idx) { for (pos, actual_col) in rr.non_structure_columns.iter().enumerate() { let choice = rr.choices.get(pos).cloned().unwrap_or(ReviewChoice::Original); if matches!(choice, ReviewChoice::AI) { if let Some(ai_val) = rr.ai.get(pos).cloned() { state.ai_throttled_apply_queue.push_back(ThrottledAiAction::UpdateCell { row_index: rr.row_index, col_index: *actual_col, value: ai_val }); } } } } }
    // New rows: only those either non-duplicates or duplicates with decided merge state
        let new_len = state.ai_new_row_reviews.len();
        for idx in 0..new_len { if let Some(nr) = state.ai_new_row_reviews.get(idx) {
            if let Some(match_row) = nr.duplicate_match_row { if !nr.merge_decided { continue; } // skip undecided
                if nr.merge_selected { // apply as updates based on choices
                    if let Some(choices) = nr.choices.as_ref() { for (pos, actual_col) in nr.non_structure_columns.iter().enumerate() { if let Some(choice) = choices.get(pos) { if matches!(choice, ReviewChoice::AI) { if let Some(val) = nr.ai.get(pos).cloned() { state.ai_throttled_apply_queue.push_back(ThrottledAiAction::UpdateCell { row_index: match_row, col_index: *actual_col, value: val }); } } } } }
                } else { // separate add
                    let mut init_vals: Vec<(usize, String)> = Vec::new(); for (pos, actual_col) in nr.non_structure_columns.iter().enumerate() { if let Some(val) = nr.ai.get(pos).cloned() { init_vals.push((*actual_col, val)); } }
                    state.ai_throttled_apply_queue.push_back(ThrottledAiAction::AddRow { initial_values: init_vals });
                }
            } else { // plain new row
                let mut init_vals: Vec<(usize, String)> = Vec::new(); for (pos, actual_col) in nr.non_structure_columns.iter().enumerate() { if let Some(val) = nr.ai.get(pos).cloned() { init_vals.push((*actual_col, val)); } } state.ai_throttled_apply_queue.push_back(ThrottledAiAction::AddRow { initial_values: init_vals }); }
        } }
        ...
        for idx in new_accept.iter() {
            if let Some(nr) = state.ai_new_row_reviews.get(*idx) {
                if let Some(match_row) = nr.duplicate_match_row {
                    if nr.merge_selected { // merge into existing row
                        if let Some(choices) = nr.choices.as_ref() { for (pos, actual_col) in nr.non_structure_columns.iter().enumerate() { if let Some(choice) = choices.get(pos) { if matches!(choice, ReviewChoice::AI) { if let Some(val) = nr.ai.get(pos).cloned() { cell_update_writer.write(UpdateCellEvent { category: selected_category_clone.clone(), sheet_name: active_sheet_name.to_string(), row_index: match_row, col_index: *actual_col, new_value: val }); } } } } }
                    } else { // separate row creation
                        let mut init_vals: Vec<(usize, String)> = Vec::new(); for (pos, actual_col) in nr.non_structure_columns.iter().enumerate() { if let Some(val) = nr.ai.get(pos).cloned() { init_vals.push((*actual_col, val)); } }
                        let req = AddSheetRowRequest { category: selected_category_clone.clone(), sheet_name: active_sheet_name.to_string(), initial_values: if init_vals.is_empty() { None } else { Some(init_vals) } }; add_row_writer.write(req);
                    }
                } else {
                    // normal new row
                    let mut init_vals: Vec<(usize, String)> = Vec::new(); for (pos, actual_col) in nr.non_structure_columns.iter().enumerate() { if let Some(val) = nr.ai.get(pos).cloned() { init_vals.push((*actual_col, val)); } }
