...
pub(super) fn draw_inline_ai_review_panel(
    ...
    let suggestion_indices_and_state = if let (Some(original_row), Some(metadata), Some((_idx, suggestion_buf))) = (original_data_cloned.as_ref(), metadata_opt, state.current_ai_suggestion_edit_buffer.as_ref()) {
        ...
        egui::Frame::group(ui.style()).inner_margin(egui::Margin::same(5)).show(ui, |ui| {
            ...
            match suggestion_indices_and_state.as_ref() {
                Some((visible_col_indices, original_row, metadata)) => {
                    ...
                    let table = TableBuilder::new(ui).striped(true).resizable(true).cell_layout(egui::Layout::left_to_right(Align::Center)).columns(Column::auto().at_least(80.0), num_cols).min_scrolled_height(0.0);
                    table.header(20.0, |mut header| { for (_display_idx, actual_idx) in visible_col_indices.iter().enumerate() { header.col(|ui| { let col_header = metadata.columns.get(*actual_idx).map_or_else(|| format!("Col {}", actual_idx+1), |c| c.header.clone()); ui.strong(col_header); }); } }).body(|mut body| {
    ...
    }, "skip" => advance_review_queue(state), "cancel" => exit_review_mode(state), _ => {} } }
}
