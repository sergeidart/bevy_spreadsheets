...
use bevy::prelude::{EventWriter, info};
...
pub fn queue_new_row_add(
    ...
    for entry in structure_entries {
        ...
        info!("Serializing structure for new row: {} rows, schema_headers={:?}", rows_to_serialize.len(), entry.schema_headers);
        let array_of_objects: Vec<serde_json::Map<String, serde_json::Value>> = rows_to_serialize
            ...
            .map(|(row_idx, row)| {
                ...
                for (i, value) in row.iter().enumerate() {
                    let field_name = entry.schema_headers.get(i).cloned().unwrap_or_else(|| format!("field_{}", i));
        ...
        info!("Adding structure to new row: col={}, json={}", col_index, json_string);
...
pub fn finalize_if_empty(state: &mut EditorWindowState) {
    ...
    if state.ai_row_reviews.is_empty() && state.ai_new_row_reviews.is_empty() && !has_undecided_structures {
...
pub fn process_existing_accept(
    ...
    for idx in sorted {
        if idx < state.ai_row_reviews.len() {
            ...
            info!("process_existing_accept: row_index={}, found {} structure entries", rr.row_index, structure_entries.len());
            for entry in structure_entries {
                ...
                // Convert rows to array of objects using schema_headers
                info!("Serializing structure: {} rows, schema_headers={:?}", rows_to_serialize.len(), entry.schema_headers);
                let array_of_objects: Vec<serde_json::Map<String, serde_json::Value>> = rows_to_serialize
                    .iter()
                    .enumerate()
                    .map(|(row_idx, row)| {
                ...
                let col_index = *entry.structure_path.first().unwrap_or(&0);
                info!("Writing structure to cell: row={}, col={}, json={}", entry.parent_row_index, col_index, json_string);
                cell_update_writer.write(UpdateCellEvent {
                    category: selected_category.clone(),
                    sheet_name: active_sheet_name.to_string(),
...
pub fn process_new_accept(
    ...
    for idx in sorted {
        if idx < state.ai_new_row_reviews.len() {
            ...
            info!("Processing new row accept: found {} structure entries for new row idx {}", structure_entries.len(), idx);
