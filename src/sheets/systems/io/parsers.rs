...
use std::fs;
use std::path::Path; // Use definition
...
pub fn read_and_parse_json_sheet(path: &Path) -> Result<(Vec<Vec<String>>, String), String> {
    ...
    let filename = path
        ...
        .unwrap_or_else(|| {
            warn!(
                "Could not derive filename from path '{}', using 'unknown.json'.",
                path.display()
            );
    ...
    if trimmed_content.is_empty() {
        warn!(
            "File '{}' is empty. Parsing as empty sheet.",
            path.display()
        );
        return Ok((Vec::new(), filename));
    }

    let grid: Vec<Vec<String>> = serde_json::from_str(trimmed_content).map_err(|e| {
        format!(
            "Failed to parse JSON grid data from '{}': {}",
            path.display(),
            e
        )
    })?;
