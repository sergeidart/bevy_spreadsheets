...
use crate::sheets::{
    ...
    events::{RequestDeleteSheetFile, RequestRenameSheetFile, RequestCreateCategoryDirectory, RequestRenameCategoryDirectory},
...
pub fn handle_create_category_directory_request(
    ...
    for ev in events.read() {
        ...
        if dir_path.exists() {
            trace!("Category directory already exists: '{}'", dir_path.display());
            continue;
        }
        match fs::create_dir_all(&dir_path) {
            Ok(_) => info!("Created category directory: '{}'", dir_path.display()),
            Err(e) => error!("Failed to create category directory '{}': {}", dir_path.display(), e),
        }
    }
}
...
pub fn handle_rename_category_directory_request(
    ...
    for ev in events.read() {
        ...
        if old.is_empty() || new.is_empty() || old == new { continue; }
        let from = base.join(old);
        let to = base.join(new);
        if from.exists() {
            if to.exists() {
                warn!("Target category directory already exists: '{}'", to.display());
                continue;
            }
            match fs::rename(&from, &to) {
                Ok(_) => info!("Renamed category directory: '{}' -> '{}'", from.display(), to.display()),
                Err(e) => error!("Failed to rename category directory '{}' -> '{}': {}", from.display(), to.display(), e),
            }
        } else {
