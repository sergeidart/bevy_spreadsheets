...
/// Should run after initial registration and loading. Triggers save for each loaded sheet.
pub fn scan_directory_for_sheets_startup(mut registry: ResMut<SheetRegistry>) {
    ...
    let mut found_unregistered_count = 0; // Still useful for final summary message
                            ...
                            if !already_registered {
                                ...
                                match load_and_parse_json_sheet(&path) {
                                    Ok((grid_data, _loaded_filename)) => {
                                        ...
                                        // *** NOTE: This modifies the registry ***
                                        ...
                                        // *** TRIGGER SAVE for this specific sheet ***
                                        info!("Sheet '{}' added during scan, triggering immediate save.", sheet_name_candidate);
                                        save_single_sheet(&registry, &sheet_name_candidate); // MODIFIED CALL
                                    }
                                    Err(e) => {
    ...
    // --- Remove save call from here, it happens inside the loop now ---
    if found_unregistered_count == 0 {
        info!("No new manually added sheets found.");
    } else {
         // Just log the count, saving happened individually
         info!("Found, processed, and saved {} manually added sheets.", found_unregistered_count);
    }
