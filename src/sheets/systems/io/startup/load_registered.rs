...
pub fn load_data_for_registered_sheets(
    ...
    if sheet_identifiers.is_empty() {
        ...
        trace!(
            "Startup Load: Found registered sheets: {:?}",
            sheet_identifiers
        );
    }
...
fn load_and_update_single_sheet_entry(
    ...
    // Returns (needs_save, load_successful)
    trace!(
    ...
    if let Some(meta_path) = &expected_meta_path_opt {
        if validate_file_exists(meta_path).is_ok() {
            match read_and_parse_metadata_file(meta_path) {
                    ...
                    match validator::validate_or_correct_loaded_metadata(
                        ...
                        Ok(()) => {
                            ...
                            if loaded_meta.category != *category {
                                warn!(
                                    "Correcting category from '{:?}' to '{:?}' for '{}'",
                                    loaded_meta.category, category, sheet_name
                                );
                            ...
                            if let Some(rp) = &loaded_meta.random_picker {
                                ...
                                trace!(
                                    "Startup Load: No RandomPicker for '{:?}/{}'",
                                    category,
                                    sheet_name
                                );
                            }
                ...
                Err(e) => {
                    error!(
                        "Failed to read/parse metadata file '{}': {}. Using registry metadata.",
                        meta_path.display(),
                        e
                    );
                }
        ...
        if !grid_filename.is_empty() {
            ...
            match grid_load::load_grid_data_file(&full_grid_path) {
                ...
                Err(e) => {
                    ...
                    return (needs_save_after_correction, false);
                }
            }
        } else {
            warn!(
                "Skipping grid load for '{:?}/{}': Filename in metadata is empty.",
                category, sheet_name
            );
            loaded_grid_data_opt = Some(Vec::new());
        }
    } else {
        warn!(
            "Cannot load grid for '{:?}/{}': No data filename identified in metadata.",
            category, sheet_name
        );
        return (needs_save_after_correction, false);
    }
    ...
    if let Some(sheet_entry) = registry.get_sheet_mut(category, sheet_name) {
        ...
        if let Some(loaded_meta) = loaded_metadata_opt {
            ...
            if meta.ensure_column_consistency() {
                info!(
                    "Corrected column consistency for existing registry metadata for '{:?}/{}'.",
                    category, sheet_name
                );
                needs_save_after_correction = true;
            }
            ...
            if Some(meta.data_filename.clone()) != final_grid_filename {
                warn!("Correcting filename in existing registry metadata for '{}' from '{}' to match determined filename ('{:?}').", sheet_name, meta.data_filename, final_grid_filename);
            ...
            let default_filename = final_grid_filename
                .clone()
                .unwrap_or_else(|| format!("{}.json", sheet_name));
            let num_cols = loaded_grid_data_opt
                .as_ref()
                .and_then(|g| g.first())
                .map_or(0, |r| r.len());
        ...
        if let (Some(grid), Some(meta)) = (&loaded_grid_data_opt, sheet_entry.metadata.as_ref()) {
            if meta.columns.is_empty() && !grid.is_empty() && grid.iter().any(|r| !r.is_empty()) {
                ...
                match validator::validate_grid_structure(grid, meta, sheet_name) {
                    ...
                    Err(e) => {
                        warn!(
                            "Grid structure validation failed for '{:?}/{}': {}. Allowing load.",
            ...
            warn!(
                "Cannot validate grid structure for '{:?}/{}': Metadata unavailable.",
                category, sheet_name
            );
            grid_validation_passed = true;
        } else {
        ...
        if grid_validation_passed {
            if let Some(grid_data) = loaded_grid_data_opt {
                sheet_entry.grid = grid_data;
                trace!("Updated grid data for '{:?}/{}'.", category, sheet_name);
            } else if final_grid_filename.is_some() {
                sheet_entry.grid = Vec::new();
                trace!(
                    "Set empty grid data for '{:?}/{}' as file was empty.",
                    category,
                    sheet_name
                );
            }
            ...
            warn!(
                "Skipping grid update for '{:?}/{}' due to structure validation failure.",
                category, sheet_name
            );
        }
                        ...
                        if let Ok(val) = serde_json::from_str::<serde_json::Value>(cell) {
                            if let serde_json::Value::Array(arr) = val {
                                if let Some(serde_json::Value::Array(inner)) = arr.first() {
                                    col.structure_schema = Some(
                                        inner
                                            .iter()
                                            .enumerate()
                                            .map(|(i, _)| StructureFieldDefinition {
                                                header: format!("Field{}", i + 1),
                                                validator: Some(ColumnValidator::Basic(
                                    ...
                                    col.structure_column_order = col
                                        .structure_schema
                                        .as_ref()
                                        .map(|s| (0..s.len()).collect());
                                    changed_local = true;
                                }
                            }
                        }
        ...
        trace!(
            ...
            sheet_entry.grid.len()
        );
    } else {
        error!(
            "Sheet '{:?}/{}' not found in registry during mutable update.",
            category, sheet_name
        );
