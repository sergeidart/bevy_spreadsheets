    ...
    let sheet_name = event.sheet_name.clone();
        ...
        if let Some(state) = editor_state.as_ref() {
            if sheet_name.starts_with("__virtual__") {
                ...
                if let Some(vctx) = state.virtual_structure_stack.iter().find(|v| v.virtual_sheet_name == sheet_name) {
                    is_virtual = true;
                    ...
                    if let Some(row) = sheet_data.grid.get_mut(row_idx) {
                        if let Some(cell) = row.get_mut(col_idx) {
                            if *cell != *new_value {
                                        ...
                                        if matches!(col_def.validator, Some(ColumnValidator::Structure)) {
                                            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&final_val) {
                                                ...
                                                let mut arr: Vec<Value> = match parsed {
                                                    ...
                                                    Value::Array(a) => a,
                                                    other => { let mut m=Map::new(); m.insert("value".into(), other); vec![Value::Object(m)] }
                                                };
                                                for v in arr.iter_mut() { if let Value::Object(o)=v { o.remove("source_column_indices"); } }
                                ...
                                trace!("Updating cell [{},{}] in sheet '{:?}/{}' from '{}' to '{}'", row_idx, col_idx, category, sheet_name, cell, final_val);
                                *cell = final_val;
            ...
            if let Some(parent_ctx) = parent_ctx_opt.clone() {
                ...
                if let Some(vsheet) = registry.get_sheet(&category, &sheet_name) {
                    if vsheet.metadata.is_some() {
                        ...
                        if let Some(parent_sheet_data) = registry.get_sheet_mut(&parent_ctx.parent_category, &parent_ctx.parent_sheet) {
                            if parent_ctx.parent_row < parent_sheet_data.grid.len() {
                                if let Some(parent_row) = parent_sheet_data.grid.get_mut(parent_ctx.parent_row) {
                                    if parent_ctx.parent_col < parent_row.len() {
                                        ...
                                        let new_json = if v_rows.len() <= 1 {
                                            ...
                                            serde_json::Value::Array(row_vals.into_iter().map(serde_json::Value::String).collect()).to_string()
                                        } else {
                                            // Multi row => array of arrays
                                            let outer: Vec<serde_json::Value> = v_rows.iter().map(|r| serde_json::Value::Array(r.iter().cloned().map(serde_json::Value::String).collect())).collect();
                                            serde_json::Value::Array(outer).to_string()
                                        };
                                        if let Some(cell_ref) = parent_row.get_mut(parent_ctx.parent_col) {
                                            if *cell_ref != new_json {
                                                *cell_ref = new_json.clone();
                                                if let Some(pmeta) = &parent_sheet_data.metadata {
                                                    let key = (parent_ctx.parent_category.clone(), parent_ctx.parent_sheet.clone());
                                                    sheets_to_save.insert(key.clone(), pmeta.clone());
                                                    sheets_to_revalidate.insert(key.clone(), ());
                                                    data_modified_writer.write(SheetDataModifiedInRegistryEvent { category: parent_ctx.parent_category.clone(), sheet_name: parent_ctx.parent_sheet.clone() });
                                                }
                                            }
                                        }
                                    }
