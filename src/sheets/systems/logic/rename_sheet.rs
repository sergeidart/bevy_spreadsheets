...
pub fn handle_rename_request(
    ...
    for event in events.read() {
        ...
        info!("Handling rename request: '{:?}/{}' -> '{:?}/{}'", category, old_name, category, new_name);
        ...
        let (old_grid_filename_opt, old_meta_filename_opt, old_category_opt) = {
            ...
            registry_immut.get_sheet(category, old_name)
            ...
            Ok(mut moved_data) => { // rename_sheet now returns the data with *updated* metadata
                ...
                if let Some(meta_to_save) = &mut moved_data.metadata {
                     ...
                     if meta_to_save.category != *category {
                         warn!("Correcting category mismatch in renamed data before save for '{}'.", new_name);
                     ...
                     // Add filenames
                     if let Some(old_grid_fn) = old_grid_filename_opt { old_grid_rel_path.push(old_grid_fn); }
                     if !new_grid_filename.is_empty() { new_grid_rel_path.push(new_grid_filename); }
                     if let Some(old_meta_fn) = old_meta_filename_opt { old_meta_rel_path.push(old_meta_fn); }
