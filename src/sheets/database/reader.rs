...
use crate::sheets::definitions::{SheetMetadata, SheetGridData, ColumnDefinition, ColumnDataType, ColumnValidator};
...
impl DbReader {
    ...
    pub fn read_metadata(conn: &Connection, table_name: &str) -> DbResult<SheetMetadata> {
        ...
        if !exists {
            return Err(DbError::TableNotFound(meta_table));
        }
        ...
        let mut stmt = conn.prepare(&format!(
            "SELECT column_index, column_name, data_type, validator_type, validator_config, ai_context, filter_expr, ai_enable_row_generation, ai_include_in_send
             FROM \"{}\" ORDER BY column_index",
            meta_table
        ))?;
        
        let columns: Vec<ColumnDefinition> = stmt.query_map([], |row| {
                ...
                Some("Linked") => {
                    if let Some(config_json) = validator_config {
                        ...
                        Some(ColumnValidator::Linked {
                            target_sheet_name: config["target_table"].as_str().unwrap_or_default().to_string(),
                            target_column_index: config["target_column_index"].as_u64().unwrap_or(0) as usize,
                        })
                    } else {
                        None
                    }
        ...
        Ok(SheetMetadata {
            ...
            hidden: hidden.map(|v| v != 0).unwrap_or_else(|| matches!(table_type.as_deref(), Some("structure"))),
        })
    }
    ...
    pub fn read_grid_data(
        ...
        let non_structure_cols: Vec<(usize, String)> = metadata.columns.iter()
            .enumerate()
            .filter(|(_, c)| !matches!(c.validator, Some(ColumnValidator::Structure)))
            .map(|(idx, c)| (idx, c.header.clone()))
            .collect();
        
        let structure_cols: Vec<(usize, String)> = metadata.columns.iter()
            .enumerate()
            .filter(|(_, c)| matches!(c.validator, Some(ColumnValidator::Structure)))
            .map(|(idx, c)| (idx, c.header.clone()))
            .collect();
        ...
        let rows = stmt.query_map([], |row| {
            ...
            let value_cols_count = if is_structure { non_structure_cols.len().saturating_sub(1) } else { non_structure_cols.len() };
            for i in 0..value_cols_count {
                let value: Option<String> = row.get(i + 1)?; // +1 because id is first
            ...
            for (col_idx, col_name) in &structure_cols {
                ...
                let count: i64 = conn.query_row(
                    &format!("SELECT COUNT(*) FROM \"{}\" WHERE parent_id = ?", structure_table),
                    [row_id],
                    |r| r.get(0)
                ).unwrap_or(0);
                // Display as human-friendly label
                let label = if count == 1 { "1 row".to_string() } else { format!("{} rows", count) };
            ...
            if is_structure {
                ...
                if let Some((id_idx, _)) = non_structure_cols.iter().find(|(_, n)| n == "id") {
                    if *id_idx < cells.len() { cells[*id_idx] = row_id.to_string(); }
                } else {
    ...
    pub fn list_sheets(conn: &Connection) -> DbResult<Vec<String>> {
        ...
        let sheets = stmt.query_map([], |row| row.get(0))?
