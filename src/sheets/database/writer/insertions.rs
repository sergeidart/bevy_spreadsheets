...
fn insert_row_with_index(
    ...
    // NOTE: We cannot reliably query back the inserted ID immediately after daemon write
    // due to WAL visibility (daemon writes to WAL, direct read connection may not see it yet).
    // Since callers don't actually use the returned ID (only check for errors), we return
    // a placeholder. The actual ID is auto-generated by SQLite and stored correctly.
    // If the ID is needed in the future, consider:
    // 1. Calculating it beforehand from MAX(id) + 1
    // 2. Using the daemon for reads as well
    // 3. Adding a WAL checkpoint after daemon writes
    Ok(-1) // Placeholder ID - callers only check for errors, not the actual value
}
...
pub fn prepend_row(
    ...
    // In WAL mode, ensure we see all committed data from daemon by doing a passive checkpoint
    // PASSIVE mode is non-blocking and just updates our view of the WAL
    let _ = conn.query_row("PRAGMA wal_checkpoint(PASSIVE)", [], |_| Ok(()))
        .map_err(|e| warn!("Failed to checkpoint WAL before reading MAX(row_index): {}", e));
    ...
    let inserted = insert_row_with_index(table_name, max_index, row_data, column_names, db_filename, daemon_client)?;
    tx.commit()?;
    Ok(inserted)
}
...
pub fn prepend_rows_batch(
    ...
    for (i, row_data) in rows_data.iter().enumerate() {
        ...
        let id = insert_row_with_index(table_name, row_index, row_data, column_names, db_filename, daemon_client)?;
